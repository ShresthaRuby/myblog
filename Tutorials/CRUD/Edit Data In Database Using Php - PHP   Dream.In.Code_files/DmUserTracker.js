...
function receiveMessage(event) {
    ...
    var sendDmGlobalUserId = function (response) {
        ...
        if (response) {
            ...
            //expire in 50 years, try set cookie.
            try{
                Cookies.SetValue(DmGlobalUserIdKey, DmGlobalUserId, 365 * 50, "/", Cookies.GetDomain(), false);
            }
            catch (e) { }

            //set local storage. it will be used as backup if cookie is wiped
            if (LocalStorage.IsEnabled()) 
                LocalStorage.SetValue(DmGlobalUserIdKey, DmGlobalUserId);            
        }        
...
function GetDmGlobalUserId(endPointURL, responseCallbackFunction) {
